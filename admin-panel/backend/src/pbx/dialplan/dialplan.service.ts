...
import { PbxDestination, PbxMetaV1 } from '../meta/pbx-meta.types';
...
export class DialplanService {
  ...
  ensurePublicIncludesDir() {
    ...
    if (
      read.content.includes('X-PRE-PROCESS cmd="include" data="public/*.xml"')
    )
      return;

    // Insert include near top of <context name="public"> so generated rules can override catch-alls.
    const marker = '<context name="public">';
    const idx = read.content.indexOf(marker);
    if (idx === -1) return;
    const insertAt = idx + marker.length;
    const patched =
      read.content.slice(0, insertAt) +
  ...
  ensureDefaultIncludesDirEarly() {
    ...
    const read = this.files.readFile(rel);
    if (
      read.content.includes('Admin Panel includes (time conditions, etc.)') &&
      read.content.includes('X-PRE-PROCESS cmd="include" data="default/*.xml"')
    ) {
      // We want it earlier than built-in queue/outbound stanzas. If it already exists twice, don't touch.
      const occurrences = (
        read.content.match(
          /X-PRE-PROCESS cmd="include" data="default\/\*\.xml"/g,
        ) || []
      ).length;
      if (
        occurrences >= 1 &&
        read.content.indexOf(
    ...
    // Remove existing include line if present (we will re-add early).
    let content = read.content.replace(
      /\s*<!-- Admin Panel includes[\s\S]*?<X-PRE-PROCESS cmd="include" data="default\/\*\.xml"\/>\s*/m,
      '\n',
    );

    // Insert after the global extension block header.
  ...
  writeTrunkInbound(meta: PbxMetaV1) {
  ...
  writeOutboundDefaults(meta: PbxMetaV1) {
    // NOTE: current dialplan ships with a single hardcoded gateway name: sip_trunk_provider
    // We apply the configured defaults for that trunk name, if present.
    const rel = 'dialplan/default/05_adminpanel_outbound_defaults.xml';
    const t = meta.trunks?.['sip_trunk_provider']?.outgoingDefault as any;

    let sound = '';
    let ivr = '';
    if (t?.type === 'sound') sound = String(t.sound ?? '');
    if (t?.type === 'ivr') ivr = String(t.ivr ?? '');
  ...
  writeQueues(meta: PbxMetaV1) {
    ...
    for (const [queueFullName, q] of Object.entries(meta.queues ?? {})) {
      if (!q?.extensionNumber || !/^\d+$/.test(q.extensionNumber)) continue;
      const qname = esc(queueFullName);
      const dn = esc(q.extensionNumber);
      const dest = q.timeoutDestination;

      // We keep the routing decision in Lua after callcenter returns.
      const postLuaArgs = dest
        ? `${escLuaArg(dest.type)} ${escLuaArg((dest as any).target ?? '')}`
        : '';
  ...
  writeOutboundPrefixRoutes(meta: PbxMetaV1) {
    // Generates outbound routing rules (prefix strip + prepend) per trunk.
    // This is included early via default/*.xml include.
    const rel = 'dialplan/default/01_adminpanel_outbound_routes.xml';
    const extNodes: string[] = [];

    for (const [trunkName, t] of Object.entries(meta.trunks ?? {})) {
      const rules = (t as any)?.prefixRules ?? [];
      const enabledRules = (rules as any[]).filter(
        (r) => r && r.prefix && r.enabled !== false,
      );
      if (enabledRules.length === 0) continue;

      const trunkEsc = esc(trunkName);
      const sound =
        (t as any)?.outgoingDefault?.type === 'sound'
          ? String((t as any)?.outgoingDefault?.sound ?? '')
          : '';
      const ivr =
        (t as any)?.outgoingDefault?.type === 'ivr'
          ? String((t as any)?.outgoingDefault?.ivr ?? '')
          : '';
...
function assertDestination(dest: PbxDestination) {
  switch (dest.type) {
    case 'terminate':
      return;
    case 'extension':
    case 'timeCondition': {
      ...
      if (!/^\d+$/.test(target)) {
        throw new BadRequestException(`Invalid ${dest.type} target "${target}". Expected digits only.`);
      }
      return;
    }
    case 'queue': {
      const target = String(dest.target ?? '').trim();
      if (!/^[^\s@]+@[^\s@]+$/.test(target)) {
        throw new BadRequestException(`Invalid queue target "${target}". Expected format like "queue1@default".`);
      }
      return;
    }
    case 'ivr': {
...
function escLuaArg(s: string) {
  // Keep args simple for lua argv parsing: strip whitespace and quotes.
