...
export class SoundsService {
    ...
    constructor(config: ConfigService) {
        const configuredFsSounds =
            config.get<string>('FS_SOUND_DIR') ?? config.get<string>('FS_SOUNDS_DIR') ?? undefined;
    ...
    ensureDir(relDir: string) {
        ...
        if (!/^[a-zA-Z0-9_./-]+$/.test(clean)) throw new BadRequestException('Invalid dir');

        const abs = path.resolve(this.baseDir, clean);
        const base = path.resolve(this.baseDir);
        if (!abs.startsWith(base + path.sep) && abs !== base) throw new BadRequestException('Invalid dir');
    ...
    sanitizeFilename(name: string) {
        ...
        if (!base.toLowerCase().endsWith('.wav')) throw new BadRequestException('Only .wav files are allowed');
        return base;
    }
