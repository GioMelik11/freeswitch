...
export class QueuesService {
  ...
  getConfig() {
    ...
    const queues = asArray(cfg?.queues?.queue).map((q: any): CallcenterQueue => {
      const name = String(q?.['@_name'] ?? '');
      const params = asArray(q?.param);
      const get = (n: string) => params.find((p: any) => p?.['@_name'] === n)?.['@_value'];
    ...
    const agents = asArray(cfg?.agents?.agent).map((a: any): CallcenterAgent => ({
      name: String(a?.['@_name'] ?? ''),
    ...
    const tiers = asArray(cfg?.tiers?.tier).map((t: any): CallcenterTier => ({
  ...
  upsertQueue(input: {
    ...
    const existingAgentNames = new Set(agents.map((a: any) => String(a?.['@_name'] ?? '')));
    ...
    const keptTiers = tiers.filter((t: any) => String(t?.['@_queue'] ?? '') !== fullName);
    const newTiers = [
      ...keptTiers,
  ...
  deleteQueue(name: string, domain?: string, etag?: string) {
    ...
    const queues = asArray(cfg?.queues?.queue).filter((q: any) => String(q?.['@_name'] ?? '') !== fullName);
    const tiers = asArray(cfg?.tiers?.tier).filter((t: any) => String(t?.['@_queue'] ?? '') !== fullName);
  ...
  private buildQueueNode(fullName: string, input: any) {
    const param = (n: string, v: string | undefined) => (v == null ? null : { '@_name': n, '@_value': v });
    const params = [
  ...
  private renderCallcenter(parts: { settings: any; queues: any[]; agents: any[]; tiers: any[] }) {
  ...
  private renderSettings(settings: any) {
    ...
    const lines = params.map((p: any) => `    <param name="${esc(p['@_name'])}" value="${esc(p['@_value'])}"/>`);
    return `<settings>\n${lines.join('\n')}\n  </settings>`;
  }
  ...
  private renderQueue(q: any) {
    ...
    const params = asArray(q?.param).map(
      (p: any) => `      <param name="${esc(p['@_name'])}" value="${esc(p['@_value'])}"/>`,
    );
