...
export class FilesService {
    ...
    constructor(config: ConfigService) {
        ...
        this.backupsDir = path.resolve(process.cwd(), config.get<string>('BACKUPS_DIR') ?? 'data/backups');
  ...
  private resolveSafe(relPath: string): ResolvedPath {
        if (!relPath || typeof relPath !== 'string') throw new BadRequestException('Invalid path');
        if (path.isAbsolute(relPath)) throw new BadRequestException('Absolute paths are not allowed');
  ...
  listFiles(relDir: string, opts?: { regex?: RegExp; extensions?: string[] }) {
    ...
    listRootTree(): TreeNode[] {
        ...
        for (const rel of roots) {
            ...
            else nodes.push({ type: 'file', name: path.basename(rel), path: rel.replace(/\\/g, '/') });
        }
        return nodes;
    }
    ...
    readFile(relPath: string) {
        ...
        if (!fs.existsSync(resolved)) throw new BadRequestException('File not found');
        const stat = fs.statSync(resolved);
        if (!stat.isFile()) throw new BadRequestException('Not a file');
        if (!this.isAllowedTextFile(resolved)) throw new BadRequestException('File type not allowed');
    ...
    writeFile(params: { path: string; content: string; etag?: string }) {
        ...
        if (exists) {
            ...
            if (etag && etag !== currentEtag) {
                throw new ConflictException('File changed since last read (etag mismatch)');
            }
